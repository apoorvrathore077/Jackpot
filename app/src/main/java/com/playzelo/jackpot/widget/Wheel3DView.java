package com.playzelo.jackpot.widget;
...
public class Wheel3DView extends View {
    ...
    public void spin(int finalIndex) {
        ...
        scroller.startScroll(0, startY, 0, targetScrollY - startY, 4000); // Reduced from 3000 to 2000
        ...
        postDelayed(() -> {
            ...
            invalidate();
        }, 2100); // Adjusted timing

        postInvalidateOnAnimation();
    }
//    public void spin(int duration,int distance,SpinListener listener){
//           isSpinning = true;
...
//    }
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        if (itemWidth == 0) itemWidth = w;
        if (itemHeight == 0) itemHeight = h;
    }

    @Override
    public void onDraw(@NonNull Canvas canvas) {
        super.onDraw(canvas);
        if (slotDrawables == null || slotDrawables.isEmpty()) return;
    ...
    public void spinSingleSlot(final int slotIndex,long durationMs){
        if (slotIndex < 0 || slotIndex >= slotDrawables.size()) return;
    ...
    public int getItemCount() {
        return slotDrawables == null ? 0 : slotDrawables.size();
